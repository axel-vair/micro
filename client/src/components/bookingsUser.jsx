...
const BookingUserList = () => {
  ...
  useEffect(() => {
    const fetchBookingsUser = async () => {
      try {
        const response = await axios.get(`http://localhost:3001/api/bookings/${userId}/myBookings`, {
          data: { userId: userId },
        });
  ...
  return (
      <div className="bg-gray-900 text-white min-h-screen">
        ...
        <div className="container mx-auto px-4 py-8">
          <h2 className="text-2xl font-bold mb-4">Liste de mes réservations</h2>
          {loading ? (
              ...
              <table className="w-full bg-gray-800 rounded-lg overflow-hidden">
                ...
                {bookings.map((booking) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-2">
                        {booking.status === true ? 'Confirmée' : 'Annulée'}
                      </td>
                      <td className="px-4 py-2">
                        <button
                            type="button"
